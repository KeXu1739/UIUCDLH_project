...
from attention_function import attention_3d_block as Attention, attention_3d_block_time_features as Attention_TS, attention_none as Attention_None
import time
...
def build_model(no_feature_cols=None, time_steps=7, output_summary=False, attnModel=Attention, LSTMunits=256):
  ...
  x = attnModel(input_layer, time_steps)
  x = Masking(mask_value=0, input_shape=(time_steps, no_feature_cols))(x) 
  x = LSTM(LSTMunits, return_sequences=True)(x)
  preds = TimeDistributed(Dense(1, activation="sigmoid"))(x)
  model = Model(inputs=input_layer, outputs=preds)
  
  # this is parameters for optimization, we do not conduct ablation on these parameters for this project 
  # since this may only impact convergence but not the structure of the model
...
def train(model_name="kaji_mach_0", synth_data=False, target='MI',
          ...
          n_percentage=1.0, time_steps=14, epochs=10, attnModel=Attention, LSTMunits=256):
  ...
  model = build_model(no_feature_cols=no_feature_cols, output_summary=True, 
                      time_steps=time_steps, attnModel=attnModel, LSTMunits=LSTMunits)

  #init callbacks
  tb_callback = TensorBoard(log_dir='./logs/{0}_{1}.log'.format(model_name, int(time.time())), # edited by Haixu, Ke
    histogram_freq=0,
    write_grads=False,
    write_images=True,
    write_graph=True) 
  ...
  model.save('./saved_models/{0}_{1}.h5'.format(model_name, int(time.time())))
...
def return_loaded_model(model_name="kaji_mach_0"):
  ...
  loaded_model = load_model("./check/{0}.h5".format(model_name))
...
if __name__ == "__main__":
    # # create pickled data for training (only need to run once), if you want to change to other time step you may change
    ...
    for target,epoch in targets_dict.items():
      for lstm in LSTMs:
        K.clear_session()
        train(model_name=f'ablation_final_no_mask_{target}_pad14_{Attention.__name__}_{lstm}', 
              epochs=epoch, synth_data=False, predict=True, target=target, time_steps=14, attnModel=Attention, LSTMunits=lstm)
