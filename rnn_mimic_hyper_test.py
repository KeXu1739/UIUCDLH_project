...
def build_model(no_feature_cols=None, time_steps=7, output_summary=False, lr = 0.001, rho = 0.9):
  ...
  RMS = optimizers.RMSprop(lr=lr, rho=rho, epsilon=1e-08)
...
def train(model_name="kaji_mach_0", synth_data=False, target='MI',
          ...
          n_percentage=1.0, time_steps=14, epochs=10, lr = 0.001, rho=0.9):
  ...
  model = build_model(no_feature_cols=no_feature_cols, output_summary=True, 
                      time_steps=time_steps, lr=lr, rho=rho)

  #init callbacks
  tb_callback = TensorBoard(log_dir='./logs/{0}_{1}_lr{2}.log'.format(model_name, time(), str(lr)), # edited by Haixu
    histogram_freq=0,
    write_grads=False,
    write_images=True,
    write_graph=True) 
...
## BIG THREE ##
    ...
    # K.clear_session()
    # train(model_name='kaji_mach_final_no_mask_MI_pad14_lr001', epochs=13,
    #       synth_data=False, predict=True, target='MI', time_steps=14, lr=0.001)
    
    # K.clear_session()
    # train(model_name='kaji_mach_final_no_mask_MI_pad14_lr005', epochs=13,
    #       synth_data=False, predict=True, target='MI', time_steps=14, lr=0.005)
    
    # K.clear_session()
    # train(model_name='kaji_mach_final_no_mask_MI_pad14_lr01', epochs=13,
    #       synth_data=False, predict=True, target='MI', time_steps=14, lr=0.01)
    
    # K.clear_session()
    # train(model_name='kaji_mach_final_no_mask_MI_pad14_lr0001', epochs=13,
    #       synth_data=False, predict=True, target='MI', time_steps=14, lr=0.0001)
    
    # K.clear_session()
    # train(model_name='kaji_mach_final_no_mask_MI_pad14_lr0005', epochs=13,
    #       synth_data=False, predict=True, target='MI', time_steps=14, lr=0.0005)
    ...
    # K.clear_session()
    # train(model_name='kaji_mach_final_no_mask_' + target + '_pad14_lr0001', epochs=13,
    #       synth_data=False, predict=True, target=target, time_steps=14, lr=0.0001)
    
    # K.clear_session()
    # train(model_name='kaji_mach_final_no_mask_' + target + '_pad14_lr0005', epochs=13,
    #       synth_data=False, predict=True, target=target, time_steps=14, lr=0.0005)

    # K.clear_session()
    # train(model_name='kaji_mach_final_no_mask_' + target + '_pad14_lr001', epochs=13,
    #       synth_data=False, predict=True, target=target, time_steps=14, lr=0.001)
    
    # K.clear_session()
    # train(model_name='kaji_mach_final_no_mask_' + target + '_pad14_lr005', epochs=13,
    #       synth_data=False, predict=True, target=target, time_steps=14, lr=0.005)
    
    # K.clear_session()
    # train(model_name='kaji_mach_final_no_mask_' + target + '_pad14_lr01', epochs=13,
    #       synth_data=False, predict=True, target=target, time_steps=14, lr=0.01)
    
    target = sys.argv[1]#'VANCOMYCIN'
    lr = float(sys.argv[2])#float("0.001")#
    rho = float(sys.argv[3])#float("0.086")#
    epochs = int(sys.argv[4])
    K.clear_session()
    train(model_name='kaji_mach_final_no_mask_' + target + '_pad14_lr' + str(lr * 1000).zfill(4) + '_rho' + str(rho * 1000).zfill(3) + "_epochs" + str(epochs), epochs=epochs,
          synth_data=False, predict=True, target=target, time_steps=14, lr=lr, rho = rho)
